<?php
ob_start();
$title = "CSU-NVB - Tâches hebdomadaires";
?>


<h1 class="center"><?= $title ?></h1>
<div class="container ">
    <div class="week">
        <div class="horizontal"><span style="font-weight: bold"> Journée</span></div>
        <div class="day col-sm">
            <div class="dayheader">Lundi</div>
            <a href="?action=Edittodo" class="over">
                <div class="hour">Fax 144 Transmission</div>
            </a>
            <a href="?action=Edittodo" class="over">
                <div class="hour">Check Ambulance et Communication</div>
            </a>
            <a href="?action=Edittodo" class="over">
                <div class="hour">Contrôle stupéfiants + Date perf. Chaudes</div>
            </a>
            <a href="?action=Edittodo" class="over">
                <div class="hour">Check bibliothèque</div>
            </a>
            <a href="?action=Edittodo" class="over">
                <div class="hour">Changer Bac chariot de nettoyage</div>
            </a>
            <a href="?action=Edittodo" class="over">
                <div class="hour">Nettoyage centrale et garage</div>
            </a>
            <a href="?action=Edittodo" class="over">
                <div class="hour">Tâches spécifique</div>
            </a>
            <a href="?action=Edittodo" class="over">
                <div class="hour">Formation</div>
            </a>
            <a href="?action=Edittodo" class="over">
                <div class="hour">Remise locaux ambulances</div>
            </a>

        </div>
        <div class="day col-sm">
            <div class="dayheader">Mardi</div>
            <a href="?action=Edittodo" class="over">
                <div class="hour">Fax 144 Transmission</div>
            </a>
            <a href="?action=Edittodo" class="over">
                <div class="hour">Check Ambulance et Communication</div>
            </a>
            <a href="?action=Edittodo" class="over">
                <div class="hour">Contrôle stupéfiants + Date perf. Chaudes</div>
            </a>
            <a href="?action=Edittodo" class="over">
                <div class="hour">Check bibliothèque</div>
            </a>
            <a href="?action=Edittodo" class="over">
                <div class="hour">Changer Bac chariot de nettoyage</div>
            </a>
            <a href="?action=Edittodo" class="over">
                <div class="hour">Nettoyage centrale et garage</div>
            </a>
            <a href="?action=Edittodo" class="over">
                <div class="hour">Tâches spécifique</div>
            </a>
            <a href="?action=Edittodo" class="over">
                <div class="hour">Formation</div>
            </a>
            <a href="?action=Edittodo" class="over">
                <div class="hour">Remise locaux ambulances</div>
            </a>
        </div>
        <div class="day col-sm">
            <div class="dayheader">Mercredi</div>
            <a href="?action=Edittodo" class="over">
                <div class="hour">Fax 144 Transmission</div>
            </a>
            <a href="?action=Edittodo" class="over">
                <div class="hour">Check Ambulance et Communication</div>
            </a>
            <a href="?action=Edittodo" class="over">
                <div class="hour">Contrôle stupéfiants + Date perf. Chaudes</div>
            </a>
            <a href="?action=Edittodo" class="over">
                <div class="hour">Check bibliothèque</div>
            </a>
            <a href="?action=Edittodo" class="over">
                <div class="hour">Changer Bac chariot de nettoyage</div>
            </a>
            <a href="?action=Edittodo" class="over">
                <div class="hour">Nettoyage centrale et garage</div>
            </a>
            <a href="?action=Edittodo" class="over">
                <div class="hour">Tâches spécifique</div>
            </a>
            <a href="?action=Edittodo" class="over">
                <div class="hour">Formation</div>
            </a>
            <a href="?action=Edittodo" class="over">
                <div class="hour">Remise locaux ambulances</div>
            </a>
        </div>
        <div class="day col-sm">
            <div class="dayheader">Jeudi</div>
            <a href="?action=Edittodo" class="over">
                <div class="hour">Fax 144 Transmission</div>
            </a>
            <a href="?action=Edittodo" class="over">
                <div class="hour">Check Ambulance et Communication</div>
            </a>
            <a href="?action=Edittodo" class="over">
                <div class="hour">Contrôle stupéfiants + Date perf. Chaudes</div>
            </a>
            <a href="?action=Edittodo" class="over">
                <div class="hour">Check bibliothèque</div>
            </a>
            <a href="?action=Edittodo" class="over">
                <div class="hour">Changer Bac chariot de nettoyage</div>
            </a>
            <a href="?action=Edittodo" class="over">
                <div class="hour">Nettoyage centrale et garage</div>
            </a>
            <a href="?action=Edittodo" class="over">
                <div class="hour">Tâches spécifique</div>
            </a>
            <a href="?action=Edittodo" class="over">
                <div class="hour">Formation</div>
            </a>
            <a href="?action=Edittodo" class="over">
                <div class="hour">Remise locaux ambulances</div>
            </a>
        </div>
        <div class="day col-sm">
            <div class="dayheader">Vendredi</div>
            <a href="?action=Edittodo" class="over">
                <div class="hour">Fax 144 Transmission</div>
            </a>
            <a href="?action=Edittodo" class="over">
                <div class="hour">Check Ambulance et Communication</div>
            </a>
            <a href="?action=Edittodo" class="over">
                <div class="hour">Contrôle stupéfiants + Date perf. Chaudes</div>
            </a>
            <a href="?action=Edittodo" class="over">
                <div class="hour">Check bibliothèque</div>
            </a>
            <a href="?action=Edittodo" class="over">
                <div class="hour">Changer Bac chariot de nettoyage</div>
            </a>
            <a href="?action=Edittodo" class="over">
                <div class="hour">Nettoyage centrale et garage</div>
            </a>
            <a href="?action=Edittodo" class="over">
                <div class="hour">Tâches spécifique</div>
            </a>
            <a href="?action=Edittodo" class="over">
                <div class="hour">Formation</div>
            </a>
            <a href="?action=Edittodo" class="over">
                <div class="hour">Remise locaux ambulances</div>
            </a>
        </div>
        <div class="day col-sm">
            <div class="dayheader">Samedi</div>
            <a href="?action=Edittodo" class="over">
                <div class="hour">Fax 144 Transmission</div>
            </a>
            <a href="?action=Edittodo" class="over">
                <div class="hour">Check Ambulance et Communication</div>
            </a>
            <a href="?action=Edittodo" class="over">
                <div class="hour">Contrôle stupéfiants + Date perf. Chaudes</div>
            </a>
            <a href="?action=Edittodo" class="over">
                <div class="hour">Check bibliothèque</div>
            </a>
            <a href="?action=Edittodo" class="over">
                <div class="hour">Changer Bac chariot de nettoyage</div>
            </a>
            <a href="?action=Edittodo" class="over">
                <div class="hour">Nettoyage centrale et garage</div>
            </a>
            <a href="?action=Edittodo" class="over">
                <div class="hour">Tâches spécifique</div>
            </a>
            <a href="?action=Edittodo" class="over">
                <div class="hour">Formation</div>
            </a>
            <a href="?action=Edittodo" class="over">
                <div class="hour">Remise locaux ambulances</div>
            </a>
        </div>
        <div class="day col-sm">
            <div class="dayheader">Dimanche</div>
            <a href="?action=Edittodo" class="over">
                <div class="hour">Fax 144 Transmission</div>
            </a>
            <a href="?action=Edittodo" class="over">
                <div class="hour">Check Ambulance et Communication</div>
            </a>
            <a href="?action=Edittodo" class="over">
                <div class="hour">Contrôle stupéfiants + Date perf. Chaudes</div>
            </a>
            <a href="?action=Edittodo" class="over">
                <div class="hour">Check bibliothèque</div>
            </a>
            <a href="?action=Edittodo" class="over">
                <div class="hour">Changer Bac chariot de nettoyage</div>
            </a>
            <a href="?action=Edittodo" class="over">
                <div class="hour">Nettoyage centrale et garage</div>
            </a>
            <a href="?action=Edittodo" class="over">
                <div class="hour">Tâches spécifique</div>
            </a>
            <a href="?action=Edittodo" class="over">
                <div class="hour">Formation</div>
            </a>
            <a href="?action=Edittodo" class="over">
                <div class="hour">Remise locaux ambulances</div>
            </a>
        </div>

        <div class="container ">
            <div class="week">
                <div class="horizontal nuitcolor"><span style="font-weight: bold">NUIT</span></div>
                <div class="day col-sm">
                    <div class="dayheader">Check de Nuit</div>
                    <a href="" class="over">
                        <div class="hour"> Contrôle stupéfiants Ambulances
                            * Morphine X4
                            * Sintenyl X6
                            Nova N°....................
                        </div>
                    </a>
                    <a href=""  class="over">
                        <div class="hour">Tâches spécifique</div>
                    </a>
                    <a href=""  class="over">
                        <div class="hour">Remise locaux Transmission</div>
                    </a>


                </div>
                <div class="day col-sm">
                    <div class="dayheader">Check de Nuit</div>
                    <a href=""  class="over">
                        <div class="hour">Contrôle stupéfiants Ambulances
                            * Morphine X4
                            * Sintenyl X6
                            Nova N°....................
                        </div>
                    </a>
                    <a href="" class="over">
                        <div class="hour">Tâches spécifique</div>
                    </a>
                    <a  href=""  class="over">
                        <div class="hour">Remise locaux Transmission</div>
                    </a>

                </div>
                <div class="day col-sm">
                    <div class="dayheader">Check de Nuit</div>
                    <a href=""  class="over">
                        <div class="hour">Contrôle stupéfiants Ambulances
                            * Morphine X4
                            * Sintenyl X6
                            Nova N°....................
                        </div>
                    </a>
                    <a href=""  class="over">
                        <div class="hour">Tâches spécifique</div>
                    </a>
                    <a href="" class="over">
                        <div class="hour">Remise locaux Transmission</div>
                    </a>
                </div>
                <div class="day col-sm">
                    <div class="dayheader">Check de Nuit</div>

                    <a href=""  class="over">
                        <div class="hour">Contrôle stupéfiants Ambulances
                            * Morphine X4
                            * Sintenyl X6
                            Nova N°....................
                        </div>
                    </a>
                    <a href="" class="over">
                        <div class="hour">Tâches spécifique</div>
                    </a>
                    <a href=""  class="over">
                        <div class="hour">Remise locaux Transmission</div>
                    </a>
                </div>
                <div class="day col-sm">
                    <div class="dayheader">Check de Nuit</div>

                    <a  href="" class="over">
                        <div class="hour">Contrôle stupéfiants Ambulances
                            * Morphine X4
                            * Sintenyl X6
                            Nova N°....................
                        </div>
                    </a>
                    <a href="" class="over">
                        <div class="hour">Tâches spécifique</div>
                    </a>
                    <a href="" class="over">
                        <div class="hour">Remise locaux Transmission</div>
                    </a>
                </div>
                <div class="day col-sm">
                    <div class="dayheader">Check de Nuit</div>

                    <a href="" class="over">
                        <div class="hour">Contrôle stupéfiants Ambulances
                            * Morphine X4
                            * Sintenyl X6
                            Nova N°....................
                        </div>
                    </a>
                    <a href="" class="over">
                        <div class="hour">Tâches spécifique</div>
                    </a>
                    <a href="" class="over">
                        <div class="hour">Remise locaux Transmission</div>
                    </a>
                </div>
                <div class="day col-sm">
                    <div class="dayheader">Check de Nuit</div>
                    <a href="" class="over">
                        <div class="hour">Contrôle stupéfiants Ambulances
                            * Morphine X4
                            * Sintenyl X6
                            Nova N°....................
                        </div>
                    </a>
                    <a href="" class="over">
                        <div class="hour">Tâches spécifique</div>
                    </a>
                    <a href="" class="over">
                        <div class="hour">Remise locaux Transmission</div>
                    </a>

                </div>
            </div>
        </div>

    </div>
</div>

<?php
$content = ob_get_clean();
require "view/gabarit.php";
?>
